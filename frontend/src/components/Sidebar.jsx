...
export default function Sidebar({
  ...
  async function handleCreate(e) {
    ...
    setBusy(true)
    try {
      await projectActions.create({ name: name.trim() })
      setName('')
      setCreating(false)
    } catch (err) {
      alert(err.message)
    ...
    <aside className="glass hidden w-64 flex-shrink-0 flex-col border-r border-[var(--glass-border)] backdrop-blur-xl backdrop-saturate-150 sm:flex">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
        ...
        <div className="flex items-center justify-between px-2 pb-1 pt-3">
          ...
          <button onClick={() => setCreating((c) => !c)} title="New project" className="rounded p-0.5 text-[var(--fg-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--fg)]">
            <Icon path={ICONS.plus} className="h-3.5 w-3.5" />
          </button>
        </div>

        {creating && (
          <form onSubmit={handleCreate} className="mb-1 flex gap-1.5 px-1">
            <input autoFocus value={name} onChange={(e) => setName(e.target.value)} maxLength={120} placeholder="Project name" className="input !py-1" />
            <button disabled={busy} className="btn btn-primary !px-2">Add</button>
          </form>
        )}
            ...
            <div key={p.id} className="rounded-md">
              <div className="group flex items-center">
                <button onClick={() => onSelectScope(p)} className={`nav-item min-w-0 flex-1 ${scope?.id === p.id ? 'active' : ''}`}>
                  <Icon path={ICONS.project} className="h-4 w-4 flex-shrink-0" />
